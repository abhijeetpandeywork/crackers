...
**Audit log retention:** to keep the Activity log fast and the database lean, `audit_log` rows older than `settings.audit.retentionDays` (default 365 days, 1–3650 allowed) are pruned by a scheduled job. The job lives in `artifacts/api-server/src/lib/audit-retention.ts`, is started from the API server's `index.ts` boot hook (`startAuditRetentionScheduler()`), runs once ~30s after boot and then every 24h, and reads the retention window from settings on every run so changes take effect without a restart. Any authenticated admin can read the window via `GET /api/v1/settings/audit`, but `PUT /api/v1/settings/audit` and `POST /api/v1/settings/audit/prune` are restricted to SUPER_ADMIN — ADMINs are themselves audited, so they must not be able to shorten retention or trigger a prune that erases evidence of their own actions. The scheduler's last result (rows deleted, cutoff, duration) is exposed via `GET /api/v1/settings/audit` as `lastPrune`.
